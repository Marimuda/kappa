...
import logging
from dataset.vertebra_db_label import VertebraDBLabel
import vtk 
import itk

logger = logging.getLogger(__name__)

class VertebraDBFactoryLabel:
    """
    ...
    def _collect_file_paths(self, directory: str, extension: str) -> List[str]:
        ...
        all_files = [os.path.join(directory, f) for f in os.listdir(directory) if f.endswith(extension)]
        files = []
        for f in all_files:
    ...
    def create_dataset(self, dataset_type: str) -> Dataset:
        ...
        return VertebraDBLabel(file_paths, loader, return_sample_id=self.return_sample_id)
